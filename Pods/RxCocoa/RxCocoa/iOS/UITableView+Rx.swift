    ...
    */
    public func items<
        ...
        return { source in
            ...
            return source.subscribeProxyDataSource(ofObject: self.base, dataSource: dataSource as UITableViewDataSource, retainDataSource: true) { [weak ingredientTableView = self.base] (_: RxTableViewDataSourceProxy, event) -> Void in
                guard let ingredientTableView = ingredientTableView else {
                    return
                }
                dataSource.tableView(ingredientTableView, observedEvent: event)
            }
        }
    }
