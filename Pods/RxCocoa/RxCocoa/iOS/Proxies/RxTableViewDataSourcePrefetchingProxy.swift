...
private final class TableViewPrefetchDataSourceNotSet
    ...
    func tableView(_ ingredientTableView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {}
...
open class RxTableViewDataSourcePrefetchingProxy
    ...
    public init(ingredientTableView: ParentObject) {
        self.ingredientTableView = ingredientTableView
        super.init(parentObject: ingredientTableView, delegateProxy: RxTableViewDataSourcePrefetchingProxy.self)
    }

    // Register known implementations
    public static func registerKnownImplementations() {
        self.register { RxTableViewDataSourcePrefetchingProxy(ingredientTableView: $0) }
    }
    ...
    public func tableView(_ ingredientTableView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {
        if let subject = _prefetchRowsPublishSubject {
            subject.on(.next(indexPaths))
        }

        (_requiredMethodsPrefetchDataSource ?? tableViewPrefetchDataSourceNotSet).tableView(ingredientTableView, prefetchRowsAt: indexPaths)
    }
